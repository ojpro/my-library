...
class FileThumbnailTrait
{
    public static function getPDFThumbnail($source, $target, $size = 1024, $page = 1)
    {
        if (file_exists($source) && !is_dir($source)): // source path must be available and not be a directory
            if (mime_content_type($source) != 'application/pdf'):
                return 'not pdf';                // source is not a pdf file returns a failure
            endif;
        ...
        return "not exists";    // the source file was not available, or Imagick didn't create a file, so returns a failure
    }
